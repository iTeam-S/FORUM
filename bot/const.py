...
Voaloany amin'izany ary dia ny lafiny HESTIKA, Ianao dia olona {' sy '.join(result_partie_1) if len(result_partie_1)<3  else ' , '.join(result_partie_1[0:-1])+' ary '+result_partie_1[-1]}\n\n{' '.join(res_1)}"
...
def resultat_partie_3(result_3,lang):
    ...
    if lang=="fr":
        return f"Et enfin LES PROFESSIONS {' et '.join(result_partie_3) if len(result_partie_3)<3  else ' , '.join(result_partie_3[0:-1])+' et '+result_partie_3[-1]} \n\n{' '.join(res_3)}"
    elif lang=="en":
